...
void handle_item_by_id(u32 item_id)
{
    ...
    case 6464021:
        ...
        sauske_batteries();
        break;
    // Disabled this because we're using this value to display if we have a key on the pause screen
    case 6464003:
        // Jump Gym Key
    ...
    case 6464000:
        // Silver key
        // We don't actually give the player an item, we just display it on the pause screen
        calculate_key_on_pause();
        break;
    case 6464001:
        // Gold Key
...
void func_80024038_24C38_hook(s32 flag)
{
    ...
    if (should_run_ap_logic())
    {
        ...
        bool flag_exists = rando_access_slotdata_raw_dict_has_member_o32(flag_to_location_handle, flag_str);
        ...
        if (flag_exists)
        {
            ...
            if (has_local_item)
            {
                // List of flags that should grant items immediately when collected in our world
                ...
                u32 num_flags = sizeof(immediate_item_flags) / sizeof(immediate_item_flags[0]);
    ...
    {
        recomp_printf("Not in a file, connected to Archipelago, or not scouted yet\n");
    }
}
...
void check_for_new_items()
{
    ...
    if (!initItems)
    {
        // D_8009F700 = randoGetHealth();
        // D_8009F704 = randoGetHealth();
    ...
    if (new_items_size > old_items_size)
    {
        ...
        // // heal when being sent booster packs
        // if (item_id == 0xA) {
        //     D_8009F700 = randoGetHealth();
        // }
        ...
        if (rando_get_sending_player(old_items_size) != rando_get_own_slot_id())
        {
            ...
            // Play the sound effect for receiving an item, we're using the coin collect sound
