...
void process_flag_actors(struct ActorInstance *actor_instance, struct ActorDefinition *resolved_actor_def, unsigned short actor_id, unsigned short actor_data_file_id, int overall_index)
{
        ...
        actor_id == 0x091 || actor_id == 0x226 || actor_id == 0x326 || actor_id == 0x345)
    {
        ...
        if (D_800C7AB2 == 0x164 && actor_id == 0x088)
        {
            ...
            // }
            struct FlagActor *new_flag = (struct FlagActor *)resolved_actor_def;
            // Just replace 0x088 with 091
            new_flag->actor_id = 0x091; // Change to door actor
            recomp_printf("Actor conversion in room 164: 0x088 -> 0x091 Door (instance %d)\n", overall_index);
            actor_instance->actor_definition = (struct ActorDefinition *)new_flag;
        }
