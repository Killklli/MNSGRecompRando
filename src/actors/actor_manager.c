...
extern unsigned int
func_80013B14_14714(unsigned short file_id); // individual file loader
extern short *get_stage_file_list(
    unsigned short stage_id); // lookup function from room_files.c
...
extern int func_802154FC_5D09CC(unsigned int player_ptr, void *object,
                                int radius);
...
extern void adjust_logical_doors(ActorInstance *actor_instance,
...
static const int num_room_configs =
    sizeof(room_configs) / sizeof(room_configs[0]);
 ...
 */
static void *overlay_redirections_add(unsigned short file_id,
                                      size_t data_size)
{
  int i;
  ...
  DEBUG_PRINTF(
      "Error: No available slots to add overlay redirection for file_id=0x%X",
      file_id);
  return NULL;
}
...
void func_8020D724_5C8BF4_hook()
{
  ...
  void *actor_data_file_buffer =
      (void *)func_800141C4_14DC4(actor_data_file_id);
  ...
  ActorInstance *actor_instances = (ActorInstance *)func_80014840_15440(
      (s32)D_80231300_5EC7D0[D_800C7AB2]->actor_instances, actor_data_file_id);
  ...
  for (int config_idx = 0; config_idx < num_room_configs; config_idx++)
  {
    if (D_800C7AB2 == room_configs[config_idx].room_id)
    {
      ...
      for (int instance_idx = 0; instance_idx < instances_to_create;
           instance_idx++)
      {
        ActorInstance *extra_instance =
            &actor_instances[regular_instance_count + instance_idx];

        // Set all values to marker value (with slight variation for multiple
        // instances)
        unsigned int marker =
            room_configs[config_idx].marker_value - instance_idx;
      ...
      ActorInstance *null_terminator =
          &actor_instances[regular_instance_count + instances_to_create];
  ...
  size_t grouped_data_offset =
      regular_instance_count * sizeof(ActorInstance) + sizeof(ActorInstance);
  ...
  for (int offset = 0; offset < max_grouped_data_size;
       offset += sizeof(ActorInstance))
  {
    ActorInstance *potential_instance =
        (ActorInstance *)(grouped_data_start + offset);
    ...
    bool reasonable_position = (potential_instance->position.x > -10000 &&
                                potential_instance->position.x < 10000 &&
                                potential_instance->position.y > -10000 &&
                                potential_instance->position.y < 10000 &&
                                potential_instance->position.z > -10000 &&
                                potential_instance->position.z < 10000);
    ...
    else if (offset > 100)
    {
      ...
      {
        if (offset + check < max_grouped_data_size &&
            grouped_data_start[offset + check] != 0)
        {
          all_zero = false;
          break;
        }
  ...
  int grouped_index = 0;
  for (int overall_index = 0; overall_index < total_instances;
       overall_index++)
  {
    ...
    else
    {
      ...
      while ((int)(grouped_index * sizeof(ActorInstance)) <
             max_grouped_data_size)
      {
        ActorInstance *potential_instance =
            (ActorInstance *)(grouped_data_start +
                              grouped_index * sizeof(ActorInstance));
        unsigned char *raw_bytes = (unsigned char *)potential_instance;

        bool has_0800_pattern =
            ((int)(grouped_index * sizeof(ActorInstance)) + 12 <
                 max_grouped_data_size &&
             raw_bytes[12] == 0x08 && raw_bytes[13] == 0x00);
        bool reasonable_position = (potential_instance->position.x > -10000 &&
                                    potential_instance->position.x < 10000 &&
    ...
    // if (overall_index >= regular_instance_count)
    // {
    //     recomp_printf("Grouped Actor Instance %d: Position (%d, %d, %d),
    //     Rotation (%d, %d, %d)\n",
    //                   overall_index,
    ...
    {
      adjust_logical_doors(actor_instance, NULL, 0, actor_data_file_id,
                           overall_index);
      continue;
    }
    ...
    ActorDefinition *resolved_actor_def =
        (ActorDefinition *)func_80014840_15440(
            (s32)actor_instance->actor_definition, actor_data_file_id);
    ...
    if (resolved_actor_def != NULL)
    {
      ...
      process_enemy_actors(actor_instance, resolved_actor_def, actor_id,
                           actor_data_file_id, overall_index);

      // Update doors that need logic adjustment to prevent softlocks
      adjust_logical_doors(actor_instance, resolved_actor_def, actor_id,
                           actor_data_file_id, overall_index);
