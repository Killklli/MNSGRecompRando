...
void adjust_logical_doors(struct ActorInstance *actor_instance, struct ActorDefinition *resolved_actor_def, unsigned short actor_id, unsigned short actor_data_file_id, int overall_index)
{
    ...
    if (D_800C7AB2 == 0x0A8)
    {
        ...
        if (marker_value >= 0xFFFFFFF0)
        {
            ...
            default: // Out of bounds - log error and skip
                DEBUG_PRINTF("Error: Instance index %d is out of bounds for room 0x0A8. Skipping instance.\n", instance_index);
                return;
            }
            ...
            DEBUG_PRINTF("Converted marker instance %d to %s actor 0x%03X in room 0x0A8 with position (%d, %d, %d)\n",
    ...
    {
        if (overall_index == 4 && !READ_SAVE_DATA(SAVE_SUPER_PASS) && actor_id == 0x241)
        {
    ...
    {
        if (overall_index == 3 && !READ_SAVE_DATA(SAVE_ACHILLES_HEEL) && actor_id == 0x242)
        {
            // Create a new locked door actor definition
            struct LockedDoorActor *new_door = (struct LockedDoorActor *)recomp_alloc(sizeof(struct LockedDoorActor));
            if (new_door != NULL)
    ...
    {
        if (overall_index == 3 && !READ_SAVE_DATA(SAVE_SUDDEN_IMPACT_MAGIC) && actor_id == 0x242)
        {
            // Create a new locked door actor definition
            struct LockedDoorActor *new_door = (struct LockedDoorActor *)recomp_alloc(sizeof(struct LockedDoorActor));
            if (new_door != NULL)
