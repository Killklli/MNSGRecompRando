...
void adjust_logical_doors(ActorInstance *actor_instance, ActorDefinition *resolved_actor_def, unsigned short actor_id, unsigned short actor_data_file_id, int overall_index) {
    ...
    if (D_800C7AB2 == 0x0A8) {
        ...
        if (marker_value >= 0xFFFFFFF0) {
            ...
            {
                LockedDoorActor *new_door = (LockedDoorActor *)recomp_alloc(sizeof(LockedDoorActor));
                if (new_door != NULL) {
            ...
            if (instance_index == 0) {
                ...
                LockedDoorActor *locked = (LockedDoorActor *)actor_instance->actor_definition;
            ...
            DEBUG_PRINTF("Converted marker instance %d to %s actor 0x%03X in room "
                         ...
                         instance_index, actor_type, actor_id_for_log, actor_instance->position.x, actor_instance->position.y, actor_instance->position.z);
        }
    // } else if (D_800C7AB2 == 0x091) {
    //     if (overall_index == 1 && !stash_initialized && resolved_actor_def != NULL) {
    //         // Stash the exit data from instance 1 for later use
    //         ExitActor *exit = (ExitActor *)resolved_actor_def;
    //         stashed_exit_data = *exit;
    ...
    //         // Set different actors and positions for each instance
    //         switch (instance_index) {
    //         case 0: // First instance - Exit Actor
    //         {
    //             ExitActor *new_exit = (ExitActor *)recomp_alloc(sizeof(ExitActor));
    //             if (new_exit != NULL) {
    //                 // Use stashed exit data
    ...
    //                 new_exit->misc_unknown = 0x10;
    //                 new_exit->direction = 0x0200; // North
    //                 // Setting ExitActor dest coords
    //                 new_exit->dest_x = 0;
    //                 new_exit->dest_y = -50;
    //                 new_exit->dest_z = -50;

    //                 actor_instance->actor_definition = (ActorDefinition *)new_exit;
    //                 actor_instance->position.x = 607;
    //                 actor_instance->position.y = -80;
    //                 actor_instance->position.z = -240;
    //             }
    //         } break;
    //         case 1: // Second instance - Locked Door
    //         {
    //             LockedDoorActor *new_door = (LockedDoorActor *)recomp_alloc(sizeof(LockedDoorActor));
    //             if (new_door != NULL) {
    //                 new_door->actor_id = 0x023F;   // Normal Door actor ID
    //                 new_door->door_design = 0x01;  // Door design 01
    //                 new_door->key_type = 0x00;     // No key required
    //                 new_door->exit_id = 0x04;      // Exit ID 04
    //                 new_door->lock_index = 0xFFFF; // Default lock values
    //                 new_door->accept_key = 0xFFFF;
    //                 new_door->padding1 = 0x0000;
    //                 new_door->padding2 = 0x0000;
    ...
    //         DEBUG_PRINTF("Converted marker instance %d to %s actor 0x%03X in room "
    //                      "0x091 with position (%d, %d, %d)\n",
    //                      instance_index, actor_type, actor_id_for_log, actor_instance->position.x, actor_instance->position.y, actor_instance->position.z);
    //     }
    } else if (D_800C7AB2 == 0x130) {
        if (overall_index == 4 && !READ_SAVE_DATA(SAVE_SUPER_PASS) && actor_id == ENTITY_SWING_DOOR) {
    ...
    } else if (D_800C7AB2 == 0x143 && actor_id == ENTITY_SWING_DOOR_SHORT) {
        ...
        if (overall_index == 3 && !READ_SAVE_DATA(SAVE_ACHILLES_HEEL) && actor_id == ENTITY_SWING_DOOR_SHORT) {
            // Create a new locked door actor definition
            LockedDoorActor *new_door = (LockedDoorActor *)recomp_alloc(sizeof(LockedDoorActor));
            if (new_door != NULL) {
                // Set up the locked door parameters as specified
        ...
        if (overall_index == 3 && (!READ_SAVE_DATA(SAVE_SUDDEN_IMPACT_MAGIC) || !READ_SAVE_DATA(SAVE_GOEMON_RECRUITED)) && actor_id == ENTITY_SWING_DOOR_SHORT) {
            // Create a new locked door actor definition
            LockedDoorActor *new_door = (LockedDoorActor *)recomp_alloc(sizeof(LockedDoorActor));
            if (new_door != NULL) {
                // Set up the locked door parameters as specified
        ...
        if (overall_index == 3 && (!IS_FLAG_SET(FLAG_DEFEATED_DHARMANYO) && !chugoku_door_unlocked) && actor_id == ENTITY_SWING_DOOR) {
            // Lock the door
            LockedDoorActor *new_door = (LockedDoorActor *)recomp_alloc(sizeof(LockedDoorActor));
            if (new_door != NULL) {
                // Set up the locked door parameters as specified
        ...
        if (overall_index == 8 && actor_id == ENTITY_INVISIBLE_WALL_THAT_BLOCKS_DOORS_UNTIL_EVENT_CONDITIONS_ARE_MET) {
        ...
        if (overall_index == 2 && !((READ_SAVE_DATA(SAVE_GOEMON_RECRUITED) && READ_SAVE_DATA(SAVE_CHAIN_PIPE)) || (READ_SAVE_DATA(SAVE_SASUKE_RECRUITED) && READ_SAVE_DATA(SAVE_SUPER_JUMP_MAGIC))) &&
            actor_id == ENTITY_DOUBLE_CROSS_SLIDING_DOOR) {
            // Create a new locked door actor definition
            LockedDoorActor *new_door = (LockedDoorActor *)recomp_alloc(sizeof(LockedDoorActor));
            if (new_door != NULL) {
                // Set up the locked door parameters as specified
