...
static void connectPressed(RecompuiResource resource,
                           const RecompuiEventData *event, void *userdata)
{
  ...
  if (event->type == UI_EVENT_CLICK)
  {
    ...
    char *password_text =
        recompui_get_input_text(connect_menu.password_textinput);
    rando_init("Mystical Ninja Starring Goemon", server_text, slot_text,
               password_text);
...
RecompuiResource create_spacer(RecompuiContext context, RecompuiResource parent,
                               float flex_grow, float flex_shrink,
                               float flex_basis)
{
...
void randoCreateAPConnectMenu()
{
  ...
  connect_menu.server_label =
      recompui_create_label(connect_menu.context, connect_menu.frame.container,
                            "Server Address:Port", LABELSTYLE_NORMAL);
  recompui_set_font_size(connect_menu.server_label, 30.0f, UNIT_DP);

  // Create a text input for the server address.
  connect_menu.server_textinput = recompui_create_textinput(
      connect_menu.context, connect_menu.frame.container);
  ...
  connect_menu.slotname_label =
      recompui_create_label(connect_menu.context, connect_menu.frame.container,
                            "Slotname", LABELSTYLE_NORMAL);
  recompui_set_font_size(connect_menu.slotname_label, 30.0f, UNIT_DP);

  // Create a text input for the slotname.
  connect_menu.slot_textinput = recompui_create_textinput(
      connect_menu.context, connect_menu.frame.container);
  ...
  connect_menu.password_label =
      recompui_create_label(connect_menu.context, connect_menu.frame.container,
                            "Password", LABELSTYLE_NORMAL);
  recompui_set_font_size(connect_menu.password_label, 30.0f, UNIT_DP);

  // Create a text input for the password.
  connect_menu.password_textinput = recompui_create_passwordinput(
      connect_menu.context, connect_menu.frame.container);
  ...
  connect_menu.connect_button =
      recompui_create_button(connect_menu.context, connect_menu.frame.container,
                             "Connect", BUTTONSTYLE_SECONDARY);
...
void ShowArchipelagoConnect() { recompui_show_context(connect_menu.context); }
...
void ap_watcher()
{
  ...
  {
    // If we're not connected but have successfully connected before, re-open
    // the context window
    if (has_connected_successfully)
    {
      ...
      has_connected_successfully =
          false; // Reset flag to avoid repeatedly opening
      ui_closed_after_setup =
          false; // Reset so UI can be closed again after reconnection
    }
  }
}
