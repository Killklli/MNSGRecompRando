...
RECOMP_PATCH s32 func_801DC27C_59818C(void *entity) {
    ...
    if (entityType == 3) {
        ...
        if (gameMode == 3) {
            ...
            // Check if controller data exists - now checks BOTH arrays (matches ASM
            // patch) Original: if (D_8015C6DC[controllerIdx] == 0) Patched:  if
            // ((D_8015C5DC_15D1DC[controllerIdx] | D_8015C6DC[controllerIdx]) == 0)
    ...
    if (inputData & 0x4000) {
        ...
        if (entityType == 3) {
            if (D_8015C604 == 1) {
                // For entityType 3 with D_8015C604 == 1, trigger action but skip sound
                // check
        ...
        if (D_8015C604 == 3) {
            // Check if counter is exactly 15 frames (0xF) to play sound (for all
            // entity types except the special case above)
    ...
    counter_check:
        ...
        if (currentCounter == 0x3C) { // 60 frames = 1 second at 60fps
        ...
        } else if (currentCounter >= 0xF) { // 15 frames = 0.25 seconds
