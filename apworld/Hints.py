...
def CompileMNSGHints(world):
    ...
    all_locations = [loc for loc in world.multiworld.get_filled_locations(world.player) if loc.item]

    # Filter out event locations and vanilla items
    all_locations = [loc for loc in all_locations if loc.address is not None and loc.item.name not in vanilla_item_names]

    # Separate progression items for major hints
    progression_locations = [loc for loc in all_locations if loc.item.classification in [ItemClassification.progression, ItemClassification.progression_skip_balancing]]
